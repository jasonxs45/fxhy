<template>
  <div :class="classes" @click="clickHandler">
    <button type="button" :class="!option ?'select-placeholder':''" >{{option?option.label:placeholder}}</button>
    <select 
      :required="required"
      :pattern="pattern"
      :value="value"
      ...
      this.$popup = new Vue({
        ...
        template: `
          <actionsheet :class="classes" :open="open" :value="value" @on-close="closeHandler" @on-click="clickHandler">
            <actionsheet-item v-for="(item,index) in options" key="'item'+index" :value="item.value" :disabled="item.disabled">{{item.label}}</actionsheet-item>
          </actionsheet>
        `,
